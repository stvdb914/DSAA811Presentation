...
drawMap <- function(strSeason, olympicCaption){
  ...
  if (grepl("Every", strSeason) ) {strSeason <- ""}
  ...
  ggplot() +
    ...
    theme(plot.margin = margin(0.2,.2,0.2,.2, "cm"),
          plot.background = element_rect(fill = "lightgrey"), legend.position = "none")
}
...
#
sportTotals <- function(strSeason){
Summer <- events %>% 
